...
def GetAlbumByArtistId(db: Session = Depends(get_db),album_repo: AlbumRepository = Depends(AlbumRepository)) -> List[GetAlbumsBySingerIdSchema]:
    return album_repo.get_albums_by_singer_id(db=db)

##Crear Album
@album_route.post("/createalbum", response_model=GetAlbumsBySingerIdSchema, status_code=status.HTTP_201_CREATED)
